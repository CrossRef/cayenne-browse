...
;; TODO Pull this into a separate project, and merge with decanter.crossref .

(defonce api-root
  (if (env :api-url)
    (str (env :api-url) "/")
    "http://api.crossref.org/v1.0/"))

(defn api-path [path] (str api-root path))
...
(defn- params [spec]
  (let [{:keys [query filter facet rows offset sample cursor]} spec]
    (cond-> {}
